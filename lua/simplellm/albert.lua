...
local name = 'albert'
local api_key = require('simplellm.utils').get_api_key(name, 'ALBERT_API_KEY')

local function get_models()    -- Get a list of available models
	local cmd = {"curl", "-s", "-H", "Authorization: Bearer " .. api_key, "https://albert.api.etalab.gouv.fr/v1/models"}
	...
	return require('simplellm.utils').filter(json_response['data'], function(e)
		if not e['type'] or e['type'] ~= 'text-generation' then
			return nil
		end
		return e['id']
	end)
